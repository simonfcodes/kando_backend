...
public abstract class TaskMapper {
    ...
    // Update Task DTO mapper

    @Mapping(target = "priority", expression = "java(dto.priority == null ? task.getPriority() : Task.Priority.valueOf(dto.priority))")
    @Mapping(target = "status", expression = "java(dto.status == null ? task.getStatus() : Task.Status.valueOf(dto.status))")
    @Mapping(target = "categories", ignore = true)
    @Mapping(target = "labels", ignore = true)
    public abstract void updateEntityFromDto(UpdateTaskDto dto, @MappingTarget Task task);

    @AfterMapping
    protected void loadRelations(UpdateTaskDto dto, @MappingTarget Task task) {
        if (dto.categoryIds != null) {
            var cats = dto.categoryIds.stream()
                    .map(id -> categoryRepo.findById(id)
                    .orElseThrow(() -> new IllegalArgumentException("Category with id " + id + " not found.")))
                    .collect(Collectors.toSet());
            task.setCategories(cats);
        ...
        if (dto.labelIds != null) {
            var labels = dto.labelIds.stream()
                    .map(id -> labelRepo.findById(id)
                            .orElseThrow(() -> new IllegalArgumentException("Label with id " + id + " not found.")))
                    .collect(Collectors.toSet());
            task.setLabels(labels);
